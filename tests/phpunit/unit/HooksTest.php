...
namespace MediaWiki\Extension\HQImageMagickResize\Tests;

use MediaWiki\Extension\HQImageMagickResize\Hooks;

/**
 * @coversDefaultClass \MediaWiki\Extension\HQImageMagickResize\Hooks
 */
class HooksTest extends \MediaWikiUnitTestCase {
	...
	public function testOnBeforePageDisplayVandalizeIsTrue() {
		$config = new \HashConfig( [
			'HQImageMagickResizeVandalizeEachPage' => true
		] );
		...
		$outputPageMock->expects( $this->once() )
			->method( 'addHTML' )
			->with( '<p>HQImageMagickResize was here</p>' );
		$outputPageMock->expects( $this->once() )
			->method( 'addModules' )
			->with( 'oojs-ui-core' );
	...
	public function testOnBeforePageDisplayVandalizeFalse() {
		$config = new \HashConfig( [
			'HQImageMagickResizeVandalizeEachPage' => false
		] );
		$outputPageMock = $this->getMockBuilder( \OutputPage::class )
			->disableOriginalConstructor()
			->getMock();
